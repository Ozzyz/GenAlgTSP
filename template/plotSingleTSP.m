function plotSingleTSP(X,Y, Path, TotalDist, gen, best_fits, mean_fits, worst_fits,save_plot,display_plot,filename)
...
%   filename: filename for the plots
  ...
  title(['Path Map -> Total Distance: ' num2str(TotalDist)]);
  xlabel('x');
  ylabel('y');
